	...
	(function() {
		...
		window.onload = function() {
			...
			<?php if (!empty($kecamatan['path'])) : ?>
				set_marker_kecamatan_content(marker_kecamatan, <?= json_encode($kecamatan) ?>, "<?= ucwords($this->setting->sebutan_kecamatan) . ' ' . $kecamatan['nama_kecamatan'] ?>", "<?= favico_kecamatan() ?>", '#isi_popup_kecamatan');
			<?php endif; ?>
			<?php if (!empty($desa['path'])) : ?>
				set_marker_desa_content(marker_kecamatan, <?= json_encode($desa) ?>, "<?= ucwords($this->setting->sebutan_kecamatan) . ' ' . $desa['nama_kecamatan'] ?>", "<?= favico_desa() ?>", '#isi_popup');
			<?php endif; ?>
			...
			//Menampilkan overlayLayers Peta Semua Wilayah
			var overlayLayers = overlayWil(marker_kecamatan, marker_dusun, marker_rw, marker_rt, "<?= ucwords($this->setting->sebutan_desa) ?>", "<?= ucwords($this->setting->sebutan_dusun) ?>");
