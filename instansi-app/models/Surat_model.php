<?php class Surat_model extends CI_Model {
	...
	{
		$sql = "SELECT u.* FROM tweb_pamong u WHERE pamong_id = ?";
		$query = $this->db->query($sql, $id);
		$data = $query->row_array();
		return $data;
	}
	...
	{
		$file_logo = APPPATH . '../' . LOKASI_LOGO_INSTANSI . $nama_logo;
	...
	{
		$data_form = LOKASI_SURAT_INSTANSI.$surat."/data_form_".$surat.".php";
	...
	{
		$data_rtf = LOKASI_SURAT_INSTANSI.$surat."/data_rtf_".$surat.".php";
	...
	{
	  $lokasi = LOKASI_SURAT_INSTANSI . $nama_surat . "/" . $komponen;
